...
export const queryMode = ['M', 'V', 'I'];

export const symbolByName = {
  Void: 0,
  PosX: 13,
  PosY: 14,
  BlobType: 15,
...
for (let i = 0; i < 27; ++i) {
  const key =
    queryMode[i % 3] +
    queryMode[Math.floor(i / 3) % 3] +
    queryMode[Math.floor(i / 9) % 3];
  queryMask[key] = i;
}
  ...
  return new Promise((fullfill, reject) => {
    ...
    const connection = {
      ...
      symbolToName: symbol => {
        ...
        return connection
          .query(false, queryMask.MMV, symbol, symbolByName.BlobType, 0)
          .then(result =>
              ...
              for (let i = 0; i < valuesAndTypes.length; i += 2) {
                ...
                switch (type) {
                  ...
                  default:
                    if (v.length === 0) {
                      ...
                      console.log(
                        `unknown type '${type}' ${typeof type} ${JSON.stringify(
                          type
                        )} for ${v} : ${avs[i]}`
                      );
                    }
                ...
                if (v !== undefined) {
                  ...
                  if (propertyName.then) {
                    promises.push(
                      propertyName.then(name => (object[name] = v))
                    );
                  } else {
    ...
    if (options.store === undefined) {
      ...
      const symatem = child_process.spawn(
        executable,
        ['--port', port, '--file', options.store],
        {
          shell: false,
          detached: false,
      ...
      */
      ...
      symatem.on('error', err =>
        reject(`Failed to start ${executable}: ${err}`)
      );
